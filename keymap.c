...
const uint16_t PROGMEM keymaps[][MATRIX_ROWS][MATRIX_COLS] = {
  [_ERMAK] = LAYOUT(L(KC_LEFT_BRACKET), KC_1, KC_2, KC_3, KC_4, KC_5, KC_6, KC_7, KC_8, KC_9, KC_0, KC_RIGHT_BRACKET, KC_HASH, LGUI_T(KC_B), LALT_T(KC_Y), LCTL_T(KC_O), LSFT_T(KC_U), L(KC_QUOTE), KC_DOUBLE_QUOTE, RSFT_T(KC_L), RCTL_T(KC_D), RALT_T(KC_W), RGUI_T(KC_V), KC_Z, LCTL_T(KC_ESC), KC_C, KC_I, KC_E, KC_A, KC_COMMA, KC_DOT, KC_H, KC_T, KC_S, KC_N, L(KC_Q), OSM(MOD_LSFT), LGUI_T(KC_G), LALT_T(KC_X), LCTL_T(KC_J), LSFT_T(KC_K), KC_MINUS, KC_MUTE, KC_F20, KC_SLASH, RSFT_T(KC_R), RCTL_T(KC_M), RALT_T(KC_F), RGUI_T(KC_P), OSM(MOD_RSFT), L(KC_GRAVE), LALT_T(KC_NO), LGUI_T(KC_NO), KC_BACKSPACE, LT(_NUM, KC_TAB), LT(_NAV, KC_ENTER), KC_SPACE, RGUI_T(KC_NO), RALT_T(KC_NO), KC_QUESTION),
